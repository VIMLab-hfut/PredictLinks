...
def load_data(data_name, network_type):
    ...
    nodes1 = np.loadtxt(file_path, dtype=int, usecols=(0))
    nodes2 = np.loadtxt(file_path, dtype=int, usecols=(1))
    # print(positive)
    nodes = []
    ...
    # np.random.shuffle(positive) 随机混乱positive
...
def learning_embedding(positive, negative, network_size, test_ratio, dimension, network_type, negative_injection=True):
    ...
    wv = model.wv
    embedding_feature, empty_indices, avg_feature = np.zeros([network_size+1, dimension]), [], 0  #有所修改，network_size=>network_size+1,
...
def link2subgraph(positive, negative, nodes_size, test_ratio, hop, network_type, max_neighbors=100):
    ...
    # print(test_pos)
    # train_pos, test_pos = positive[:-6221], positive[-6221:]
    # train_neg, test_neg = negative[:-6221], negative[-6221:]
    # train_file_path = "./raw_data/train.txt"
    # test_file_path = "./raw_data/test.txt"
    # train_pos = np.loadtxt(train_file_path, dtype=int, usecols=(0, 1))
    # test_pos = np.loadtxt(test_file_path, dtype=int, usecols=(0, 1))
    # test_size = int(len(test_pos))
    # train_neg, test_neg = negative[:-test_size], negative[-test_size:]


    A = np.zeros([nodes_size+1, nodes_size+1], dtype=np.uint8) #有所修改，node_size => node_size+1
    ...
    # print((set(positive[:, 0]) | set(positive[:, 1]) | set(negative[:, 0]) | set(negative[:, 1])))
    ...
    # print(graphs_adj, labels, vertex_tags, node_size_list, sub_graphs_nodes, tags_size)
...
def classifier(data_name, is_directed, test_ratio, dimension, hop, learning_rate, top_k=60, epoch=100):
    ...
    # print("nodes_size_list",nodes_size_list)
    D_inverse_train, D_inverse_test, A_tilde_train, A_tilde_test, X_train, X_test, Y_train, Y_test, \
    nodes_size_list_train, nodes_size_list_test = split_train_test(D_inverse, A_tilde, X, Y, nodes_size_list, test_ratio)
    # print(nodes_size_list_test[-3])
    # print("在这里！！！！",nodes_size_list_train, nodes_size_list_test)
    ...
    # print("nodes_size_list_test长度",len(nodes_size_list_test))
    
    # 计算预测结果与真实情况的正例、负例个数
    P=N=TP=FP=TN=FN=0

    for i in range(len(nodes_size_list_test)):
        if(prediction[i][0] == 0):
            N += 1
        else:
            P += 1
    for i in range(len(nodes_size_list_test)):
        if(prediction[i][0] == int(Y_test[i]) == 1):
            TP += 1
        elif(prediction[i][0] == 1 and int(Y_test[i]) == 0):
            FP += 1
        elif(prediction[i][0] == 0 and int(Y_test[i]) == 1):
            FN += 1
        elif(prediction[i][0] == int(Y_test[i]) == 0):
            TN += 1
    print("P,N,TP,FP,TN,FN",P,N,TP,FP,TN,FN)
    Precision = TP/(TP+FP)
    Recall = TP/(TP+FN)
    ...
    # print("num0:",num0)
    # print("num1:",num1)
    # print("Y_test长度",len(Y_test))
    # print("X_test",X_test)
    # print("Y_test",Y_test)
    # print("prediction 长度",len(prediction))
    # print("prediction",prediction)
    # print("pos_scores 长度",len(pos_scores))
